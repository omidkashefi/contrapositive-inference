...
class CtrlGenModelConcise(object):
    """Control
    """

    def __init__(self, inputs, vocab, gamma, lambda_g, lambda_c, hparams=None):
        self._hparams = tx.HParams(hparams, None)
        self._build_model(inputs, vocab, gamma, lambda_g, lambda_c)

    def _build_model(self, inputs, vocab, gamma, lambda_g, lambda_c):
        """Builds the model.
        """
        embedder = WordEmbedder(
            vocab_size=vocab.size,
        ...
        # Conciseness loss
        refs = inputs['text_ids'][:, 1:]
        hyps = outputs_.logits

        mm = tf.minimum(tf.shape(refs), tf.shape(hyps)[:2])
        refs = refs[:,:mm[1]]
        hyps = hyps[:,:mm[1]:]

        loss_c = tx.losses.sequence_sparse_softmax_cross_entropy(
            labels=refs,
            logits=hyps,
            sequence_length=inputs['length'] - 1,
            average_across_timesteps=True,
        ...
        # Aggregates losses
        loss_g = loss_g_ae + lambda_c * loss_c + lambda_g * loss_g_clas
        loss_d = loss_d_clas
...
class CtrlGenModelPTE(object):
    ...
    def _build_model(self, inputs, vocab, gamma, lambda_g):
        """Builds the model.
        """
        embedding = Embedding(vocab.token_to_id_map_py, hparams=self._hparams.embedding)
        embedder = WordEmbedder(init_value=embedding.word_vecs, hparams=self._hparams.embedder)
